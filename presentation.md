...
- **Bigger picture: the strategies dividing into two camps.** *Repair* strategies (correction, voting) trim the exec_error tail and give small but reliable lifts. *Replace* strategies (CoT, original-buggy-linking) modify the full prompt structure and regress on strong coder bases. With the linking bug fixed, we'll see if that camp is genuinely "doesn't help on already-confident bases" or if it was the bug all along.
